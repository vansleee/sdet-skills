...
- 排除：`environment` 類的紅（若 `triage-reports/` 有標）——那是 infra 問題不是測試修復時間，計入會失真。
...
```
- 來源：**直接引用** `flaky-registry.yaml`（`flaky-manager` 維護），不自行重跑或重算重現率。
...
```
- 來源：`triage-reports/<date>_<run>.md`（`pipeline-triage` 產）＋ `issues-index.yaml` ＋ `gh issue view --json assignees,createdAt`。
- 沒開單的紅（自動判為 flaky / environment）不列入。
- 這是唯一一個**主要衡量人與流程、而非測試品質**的指標；超標通常是人力或分派規則問題，路由給人。

## quarantine_count — 隔離中的測試數
**定義**：`flaky-registry.yaml` 中 `status: quarantined` 的筆數；另記 `expired`（`status: escalated` 或 `expires_at` 已過）。

- 來源：直接數 registry。
- **`expired > 0` 一律 alert**，不管總數多少——逾期未處置的隔離就是安靜消失的覆蓋率。

## gate_pass_rate / override_count — 放行健康度
**定義**：`pipeline-gate.yaml` 中 `verdict == PASS` 的比例；`override_count` 為 `verdict == OVERRIDE` 的筆數。

- 來源：直接數 `pipeline-gate.yaml`。
- **override 變多的解讀**：不是「人在作弊」，而是「閘門準則與現實脫節」的訊號——例如把不穩定的套件列進 `required_suites`。路由給人檢視 config，而不是路由去收緊閘門。

## coverage_gap（可選）
**定義**：`traceability.yaml` 中未被任何測試或 finding 覆蓋的需求數。
