...
- **「找的人」不能當「判的人」。** Hunter 對自己找到的東西有確認偏誤，所以它的權力被刻意切窄：只輸出「候選」。獨立蓋章交 `bug-verifier`（沒有 Hunter 記憶的 subagent），能不能開單交 `issue-quality-gate`。這是第四週最重要的治理設計。
- **user-invoked。** 「派它去獵一輪」是有意識的委派，會消耗預算、會操作產品，不該由模型隱式觸發。
