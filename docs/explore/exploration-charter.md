...
- **`auth_context` 要求一個「別人」。** 授權邊界是 API 最常見也最貴的一類 bug，而它需要第二組憑證才驗得動。談章程的時候問，比探索到一半才發現沒帳號好。填 `none` 也可以，但那等於明說這一輪不驗越權。
