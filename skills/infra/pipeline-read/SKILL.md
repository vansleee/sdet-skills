...
4. **需要才下載 artifact**：要逐筆 nodeid、要 trace 時才 `gh run download <id> -n test-results-json`（或 `-n traces`）。API job 的結果在 `api-test-results-json`、證據在 `api-evidence-<slug>`，兩個都要單獨拉。artifact 名照 `references/artifact-contract.md`；有 matrix 或重跑時名稱帶 `-<matrix 值>-attempt-<n>` 後綴，改用 `--pattern "*-attempt-<n>"` 只拉同一次嘗試，別混到別次。
