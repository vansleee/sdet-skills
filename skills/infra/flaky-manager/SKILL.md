...
## 輸入 / 輸出
- **輸入**：`pipeline-triage` 判為 flaky 的群、`flaky-detect` 的定性結果、跨 run 歷史（`pipeline-read` 拉近 N 個 run，N 讀 `config/sdet-config.yaml` 的 `flaky.lookback_runs`，預設 20）、現有 `output/flaky-registry.yaml`。
