...
> 狀態檔：`pipeline-gate.yaml`（**注意**：與 `issue-quality-gate` 的 `gate.yaml` 是不同檔案，別混用）。

## 輸入 / 輸出
- **輸入**：候選識別（PR 編號 / sha / tag）＋ `pipeline-read` 的 run 結果 ＋ `flaky-registry.yaml` 隔離名單 ＋ open blocker issue（`gh issue list --label blocker`）＋ 準則（`config/sdet-config.yaml` 的 `gate`）。
- **輸出**：裁決（`PASS` / `FAIL` / `OVERRIDE`）＋ 逐條評估表（每條附證據出處）＋ 寫入 `pipeline-gate.yaml` 的留痕。
...
| 3 | `no_open_blocker` | 無 open 的 blocker label issue 指向此範圍 | `gh issue list` + `issues-index.yaml` |
| 4 | `quarantine_within_budget` | 隔離中的測試數 ≤ `gate.max_quarantined` | `flaky-registry.yaml` |
| 5 | `no_expired_quarantine` | 無逾期未處置的隔離（`status: escalated`） | `flaky-registry.yaml` |
...
6. **留痕**：寫 `pipeline-gate.yaml`（先給人看再寫）。
