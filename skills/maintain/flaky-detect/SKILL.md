---
name: flaky-detect
description: 判定一支測試是真的 flaky 還是穩定壞掉：同碼同環境重跑 N 次，量化重現率、歸類疑似根因、給下一步。當某支時紅時綠，或 re-run-gate 裁決為 flaky 需要定性時使用。關鍵詞：flaky、間歇失敗、重現率、時好時壞、不穩定、隔離。
---
...
## 疑似根因分類
| suspected_root_cause | 訊號 | 後續 |
|---|---|---|
| `wait-condition` | 等待條件不正確（元素存在但還不可用）| `test-heal` |
| `data-pollution` | 測試間資料殘留／互相污染 | `test-data` |
| `parallel-race` | 平行執行搶同一筆資料或同一個帳號 | `test-data` / `test-parallelize` |
| `external-dependency` | 依賴外部服務、時間、時區、亂數 | 隔離或 mock，交 `test-heal` |
| `render-timing` | 動畫、非同步渲染、重繪造成的時序 | `test-heal` |

## 鐵則
- **禁止**以「加 retry」或「加 sleep」作為結論——那是掩蓋不是修復。
- 本 skill 只定性、不修：修法交 `test-heal`，驗穩交 `re-run-gate`。
